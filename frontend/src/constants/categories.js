export const CATEGORIES = [
  {
    label: 'ÁO CLB, ĐT', slug: 'ao-clb-dt',
    children: [
      { label: 'JP', slug: 'jp' },
      { label: 'HD', slug: 'hd' },
      { label: 'DK', slug: 'dk' },
      { label: 'MK', slug: 'mk' },
      { label: 'Mê Caro', slug: 'me-caro' },
      { label: 'Áo Trẻ Em CLB, ĐT', slug: 'ao-tre-em-clb-dt' },
      { label: 'Strivend', slug: 'strivend' },
      { label: 'Winsport', slug: 'winsport' },
    ],
  },
  {
    label: 'GIÀY BÓNG ĐÁ', slug: 'giay-bong-da',
    children: [
      { label: 'Giày Wika', slug: 'giay-wika' },
      { label: 'Giày Kamito', slug: 'giay-kamito' },
      { label: 'Giày Jogarbola', slug: 'giay-jogarbola' },
      { label: 'Giày Akka', slug: 'giay-akka' },
      { label: 'Giày Mira', slug: 'giay-mitre' },
      { label: 'Giày Zocker', slug: 'giay-zocker' },
      { label: 'Giày Trẻ Em', slug: 'giay-tre-em' },
    ],
  },
  {
    label: 'QUẢ BÓNG ĐÁ', slug: 'qua-bong-da',
    children: [
      { label: 'Bóng Động Lực', slug: 'bong-dong-luc' },
      { label: 'Bóng Wika', slug: 'bong-wika' },
      { label: 'Bóng Zocker', slug: 'bong-zocker' },
      { label: 'Bóng Bách Hiền', slug: 'bong-bach-hien' },
      { label: 'Bóng Thiên Long', slug: 'bong-thien-long' },
    ],
  },
  {
    label: 'GIÀY CẦU LÔNG - BC - PK', slug: 'giay-cau-long-bc-pk',
    children: [
      { label: 'Giày Thể Thao Wika', slug: 'giay-tt-wika' },
      { label: 'Giày Thể Thao Lefus', slug: 'giay-tt-lefus' },
      { label: 'Giày Thể Thao Sao Vàng', slug: 'giay-tt-sao-vang' },
      { label: 'Giày Thể Thao Beyono', slug: 'giay-tt-beyono' },
    ],
  },
  {
    label: 'ÁO THỂ THAO - POLO', slug: 'ao-the-thao-polo',
    children: [
      { label: 'Áo Polo', slug: 'ao-polo' },
      { label: 'Quần Áo Thể Thao', slug: 'quan-ao-the-thao' },
    ],
  },
  {
    label: 'PICKLEBALL', slug: 'pickleball',
    children: [
      { label: 'Vợt Pickleball', slug: 'vot-pickleball' },
      { label: 'Phụ Kiện Pickleball', slug: 'phu-kien-pickleball' },
      { label: 'Giày Pickleball', slug: 'giay-pickleball' },
    ],
  },
  {
    label: 'PHỤ KIỆN BÓNG ĐÁ', slug: 'phu-kien-bong-da',
    children: [
      { label: 'Túi Đựng Bóng', slug: 'tui-dung-bong' },
      { label: 'Băng Bảo Vệ', slug: 'bang-bao-ve' },
      { label: 'Tất Thể Thao', slug: 'tat-the-thao' },
      { label: 'Găng Tay Thủ Môn', slug: 'gang-tay-thu-mon' },
      { label: 'Bình Xịt Thể Thao', slug: 'binh-xit-the-thao' },
    ],
  },
  {
    label: 'BÓNG CHUYỀN', slug: 'bong-chuyen',
    children: [
      { label: 'Quả Bóng Chuyền', slug: 'qua-bong-chuyen' },
      { label: 'Áo Bóng Chuyền', slug: 'ao-bong-chuyen' },
      { label: 'Giày Bóng Chuyền', slug: 'giay-bong-chuyen' },
      { label: 'PK Bóng Chuyền', slug: 'pk-bong-chuyen' },
    ],
  },
  {
    label: 'GIÀY CHẠY BỘ', slug: 'giay-chay-bo',
    children: [
      { label: 'Giày Chạy Bộ Nam', slug: 'giay-chay-bo-nam' },
      { label: 'Giày Chạy Bộ Nữ', slug: 'giay-chay-bo-nu' },
      { label: 'Giày Chạy Bộ Trẻ Em', slug: 'giay-chay-bo-tre-em' },
    ],
  },
  {
    label: 'QUẦN ÁO BÓNG ĐÁ THƯƠNG HIỆU', slug: 'quan-ao-bong-da-thuong-hieu',
    children: [
      { label: 'Wika', slug: 'qabd-wika' },
      { label: 'Riki', slug: 'qabd-riki' },
      { label: 'Bulbal', slug: 'qabd-bulbal' },
      { label: 'Beyono', slug: 'qabd-beyono' },
      { label: 'Kamito', slug: 'qabd-kamito' },
      { label: 'Mikai', slug: 'qabd-mikai' },
      { label: 'HD', slug: 'qabd-hd' },
      { label: 'Justplay', slug: 'qabd-justplay' },
      { label: 'Dullkill', slug: 'qabd-dullkill' },
      { label: 'CP-Egan-Win', slug: 'qabd-cp-egan-win' },
      { label: 'CV', slug: 'qabd-cv' },
      { label: 'Trẻ Em Thương Hiệu', slug: 'qabd-tre-em' },
      { label: 'Zocker', slug: 'qabd-zocker' },
      { label: 'Jogarbola', slug: 'qabd-jogarbola' },
      { label: 'CR', slug: 'qabd-cr' },
    ],
  },
];
